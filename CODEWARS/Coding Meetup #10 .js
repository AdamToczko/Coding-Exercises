...
const date = new Date();
const z6 = list1.map(v => ({...v, username: `${v.firstName}`.toLowerCase()+`${v.lastName}`.toLowerCase()+(date.getFullYear()`${v.age}`)}))
//not finished this way - need to remove dot?
...
function addUsername(list) {
    ...
    for (let i = 0; i < list.length; i++) {
      let first = list[i].firstName.toLowerCase(),
