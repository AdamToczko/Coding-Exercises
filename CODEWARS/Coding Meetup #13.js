...
const lang = [list.filter(a=>a.language==='Python').length,list.filter(a=>a.language==='Ruby').length,list.filter(a=>a.language==='JavaScript').length];

return Math.max.apply(null,lang)/Math.min.apply(null,lang) <= 2;

// different option
function isLanguageDiverse(list) {
    const lang = [list.filter(a=>a.language==='Python').length,list.filter(a=>a.language==='Ruby').length,list.filter(a=>a.language==='JavaScript').length];
     if(lang[0]<=2*lang[1] && lang[0]<=2*lang[2] && lang[2]<=2*lang[0] && lang[1]<=2*lang[2] && lang[1]<=2*lang[0] && lang[2] <=2*lang[1])
     {return true}
     else 
     {return false}
   }